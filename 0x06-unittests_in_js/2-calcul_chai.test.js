...
describe('calculateNumber', () => {
  describe('type !== "SUM" || type !== "SUBTRACT" || type !== "DIVIDE"', () => {
    it('equal positive numbers with different type of PLUS', () => {
      expect(calculateNumber('PLUS', 2.0, 2.0)).to.be.equal(0);
    });

    it('equal positive numbers (alternate) with type DIV', () => {
      expect(calculateNumber('DIV', 2.3, 1.8)).to.be.equal(0);
    });

    it('equal negative numbers with type SUB', () => {
      expect(calculateNumber('SUB', -2.0, -2.0)).to.be.equal(0);
    });

    it('equal negative numbers (alternate) with type MUL', () => {
      expect(calculateNumber('MUL', -2.3, -1.8)).to.equal(0);
    });
  });
